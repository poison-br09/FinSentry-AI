...
const UploadStatement = ({ files, setFiles, dropzoneDisabled, isProcessing }) => {
  ...
  return (
    ...
    >
      ...
      <span className="font-semibold text-gray-700 text-lg mb-1">
        {isProcessing ? "Processing..." : (files.length ? files.map(f => f.name).join(", ") : "Choose file")}
      </span>
      <span className="text-gray-500 text-base">
        {isProcessing ? "Please wait while we analyze your statement" : "Drag and drop a file here, or click to select"}
      </span>
      <input
        id="file-upload"
        type="file"
